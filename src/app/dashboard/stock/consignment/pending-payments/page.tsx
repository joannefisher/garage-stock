...
export default async function PendingPaymentsPage(
  props: PageProps<"/dashboard/stock/consignment/pending-payments">
) {
  const searchParams = (await props.searchParams) as PendingPaymentsSearchParams

  // Gated the same way as Receive stock / Receive consignment stock — this
  ...
  if (!staff?.canManageStock) {
    redirect(
      `/dashboard/stock?error=${encodeURIComponent(
        "Only admins and managers can view pending consignment payments."
      )}`
    )
  }
  ...
  // view, same redirect_to pattern as the item detail page's consignment
  // actions.
  ...
  const redirectTo = `/dashboard/stock/consignment/pending-payments${
  ...
  return (
    <div className="flex flex-col gap-4">
      <div className="flex flex-wrap items-end justify-between gap-3">
        <div>
          <h1 className="text-2xl font-bold tracking-tight">Pending consignment payments</h1>
          <p className="text-muted-foreground">
            Consignment stock that&apos;s been committed and is now owed to the supplier —
