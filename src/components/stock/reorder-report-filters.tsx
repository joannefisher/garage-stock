...
export function ReorderReportFilters({ suppliers }: { suppliers: SupplierOption[] }) {
  ...
  function apply(formData: FormData) {
    ...
    startTransition(() => {
      router.push(`/dashboard/stock/reorder-report?${params.toString()}`)
    })
  }
  ...
  return (
    <form
      action={apply}
      className="grid grid-cols-1 gap-3 rounded-2xl border bg-card p-5 sm:grid-cols-2 lg:grid-cols-4"
    >
      ...
      <div className="flex flex-col gap-1.5">
        ...
        <select
          id="sort"
          name="sort"
          defaultValue={searchParams.get("sort") ?? "urgent"}
          className={selectClass}
        >
          <option value="urgent">Most needed first</option>
          <option value="id_number">ID (A–Z)</option>
